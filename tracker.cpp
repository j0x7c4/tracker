...
void Tracker::GetDepthStream ( ) {
  ...
  while ( is_running ) {
    ...
    result = WaitForSingleObject(read_mutex_,INFINITE);
	  if ( result != WAIT_OBJECT_0 && result != WAIT_ABANDONED ) {
	    cout<<"Wait error!"<<endl;
	    return;
	  }
...
void ReadDepthLockedRect(KINECT_LOCKED_RECT& locked_rect, int w, int h, BYTE * data) {
	...
	if( locked_rect.Pitch != 0 ) {
		...
		for( int y = 0 ; y < h ; y++ ) {
			for( int x = 0 ; x < w ; x++ ) {
				RGBQUAD quad = NuiShortToQuadDepth( *p_buffer_run );
        p_buffer_run++;
				index = (y*w+x)*3;
				*(data+index + blue_idx) =quad.rgbBlue;
				*(data+index + green_idx) = quad.rgbGreen;
				*(data+index + red_idx) = quad.rgbRed;
			}
		}
	}
}
...
void Tracker::ImageProcess( ) {
  ...
  while ( is_running ) {
    ...
    */
    ...
    result = WaitForSingleObject(write_mutex_,INFINITE);
    if ( result != WAIT_OBJECT_0 && result != WAIT_ABANDONED ) {
	    cout<<"Wait error!"<<endl;
	    return;
	  }
